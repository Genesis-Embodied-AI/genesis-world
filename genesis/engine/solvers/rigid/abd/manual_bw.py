...
function theorem). The chain-rule building blocks (quaternion product, quaternion transform, rotation-vector
conversion, motion cross product) live next to their forwards in genesis/utils/geom.py as the *_grad_* adjoint funcs.
"""
...
def func_forward_kinematics_link_bw(
    ...
    """Reverse the forward kinematics of link i_l of env i_b.

    Its pose grads flow back to its joint coordinates and to its parent's pose grads (see
    ...
    # Grad seeded on the final link pose (= slot n_joints), carried backward through the joint chain: after the loop it
    # holds the grad on slot 0 (arm base)
    ...
    for k_rev in range(n_joints):
        ...
        if joint_type == gs.JOINT_TYPE.FREE:
            # Final pose is set absolutely from qpos (slot in unused), xanchor = qpos[0:3]. The forward also publishes
            # the pose as dofs.pos = (pos, quat_to_xyz(quat)), whose grad (accumulated by the force-stage reverses
            # through position control / joint stiffness) folds back here.
            for j in qd.static(range(3)):
                rigid_info.qpos.grad[q_start + j, i_b] = (
                    rigid_info.qpos.grad[q_start + j, i_b]
                    + g_pos[j]
                    + xanchor_grad[j]
                    + dyn_state.dofs.pos.grad[dof_start + j, i_b]
                )
            # The forward normalizes the raw qpos quaternion (quat_ = q / |q|) before writing the link quaternion, so
            # the gradient must pass through the Jacobian of q / |q|: J^T g = (g - qhat (qhat . g)) / |q|. Copying g
            # straight into the raw entries leaves a spurious radial component that disagrees with finite differences
            # even at unit length.
            q_raw = qd.Vector(
                [
            ...
            g_xyz = qd.Vector(
                [
                    dyn_state.dofs.pos.grad[dof_start + 3, i_b],
                    dyn_state.dofs.pos.grad[dof_start + 4, i_b],
                    dyn_state.dofs.pos.grad[dof_start + 5, i_b],
                ],
                dt=gs.qd_float,
            )
            g_quat_eff = g_quat + gu.qd_quat_to_xyz_grad_quat(qhat, rigid_info.EPS[None], g_xyz)
            g_quat_raw = (g_quat_eff - qhat * qhat.dot(g_quat_eff)) / q_norm
            for j in qd.static(range(4)):
                rigid_info.qpos.grad[q_start + 3 + j, i_b] = rigid_info.qpos.grad[q_start + 3 + j, i_b] + g_quat_raw[j]
            for j in qd.static(range(6)):
                dyn_state.dofs.pos.grad[dof_start + j, i_b] = 0.0
            ...
            # The forward also publishes the angle as dofs.pos[dof_start] = qpos - qpos0, whose grad (accumulated by the
            # force-stage reverses) folds back linearly.
            rigid_info.qpos.grad[q_start, i_b] = (
                rigid_info.qpos.grad[q_start, i_b] + angle_grad + dyn_state.dofs.pos.grad[dof_start, i_b]
            )
            dyn_state.dofs.pos.grad[dof_start, i_b] = 0.0
            ...
            # The forward also publishes the displacement as dofs.pos[dof_start] = qpos - qpos0, whose grad (accumulated
            # by the force-stage reverses) folds back linearly.
            rigid_info.qpos.grad[q_start, i_b] = (
                rigid_info.qpos.grad[q_start, i_b] + displacement_grad + dyn_state.dofs.pos.grad[dof_start, i_b]
            )
            dyn_state.dofs.pos.grad[dof_start, i_b] = 0.0
            g_xaxis = qd.Vector(
                [
            ...
            # pos_out never reads xanchor here (pos_out = pos_in + xaxis * displacement), so only the downstream xanchor
            # consumer's grad flows through xanchor = T(joint_pos_off, quat_in) + pos_in into quat_in, and g_pos itself
            # reaches pos_in directly.
            g_quat_in = (
                g_quat
                + gu.qd_transform_by_quat_grad_quat(axis, quat_in, g_xaxis)
                + gu.qd_transform_by_quat_grad_quat(joint_pos_off, quat_in, xanchor_grad)
            )
            g_pos = g_pos + xanchor_grad
            g_quat = g_quat_in
            ...
            g_qloc = gu.qd_quat_mul_grad_rhs(quat_in, qloc, gq_out)
            g_quat_in_apply = gu.qd_quat_mul_grad_lhs(quat_in, qloc, gq_out)
            # The forward also publishes the joint coordinate as dofs.pos = quat_to_xyz(qloc), whose grad (accumulated
            # by the force-stage reverses) folds back through the Euler extraction.
            g_xyz = qd.Vector(
                [
                    dyn_state.dofs.pos.grad[dof_start, i_b],
                    dyn_state.dofs.pos.grad[dof_start + 1, i_b],
                    dyn_state.dofs.pos.grad[dof_start + 2, i_b],
                ],
                dt=gs.qd_float,
            )
            g_qloc = g_qloc + gu.qd_quat_to_xyz_grad_quat(qloc, rigid_info.EPS[None], g_xyz)
            for j in qd.static(range(4)):
            ...
            # A fixed joint passes the pose through unchanged
            pass
    ...
    if i_parent != -1:
        parent_quat = dyn_state.links.quat[i_parent, i_b]
        ...
        for j in qd.static(range(3)):
            dyn_state.links.pos.grad[i_parent, i_b][j] = dyn_state.links.pos.grad[i_parent, i_b][j] + g_pos[j]
        for j in qd.static(range(4)):
            dyn_state.links.quat.grad[i_parent, i_b][j] = (
                dyn_state.links.quat.grad[i_parent, i_b][j]
                + parent_quat_grad_from_pos[j]
                + parent_quat_grad_from_quat[j]
            )

    # The forward skips writing fixed root links' pose so users can overwrite it (see func_forward_kinematics_link):
    # their pose is persistent input state, so its grad keeps accumulating across substeps and is left unconsumed here.
    if not (i_parent == -1 and dyn_info.links.is_fixed[I_l]):
        for j in qd.static(range(3)):
...
def kernel_manual_forward_kinematics_bw(
    ...
    Iterates the links of each kinematic root leaf to root, so a child's parent pos / quat grad write lands before the
    parent's own iteration consumes it, and within each link reverses the full joint chain. A link may carry several
    joints (e.g. a planar floating base = slide-x + slide-z + hinge-y on one link). The forward composes them in
    sequence and caches the per-joint intermediate pose in dyn_state.links.{pos,quat}_bw[i_l, k]: slot 0 is the parent
    pose composed with the link's fixed offset, slot k+1 the pose after joint k, slot n_joints the final link pose. The
    reverse seeds the grad on the final pose, reverses joint k for k = n_joints-1 .. 0 (each step consumes the grad on
    slot k+1 and emits qpos.grad for that joint plus the grad on slot k), then reverses the base composition into the
    parent's pose grad. Each joint also feeds dyn_state.joints.{xanchor,xaxis} downstream (forward velocity), so their
    accumulated grads are folded back through slot k as well.
    """
    ...
    for i_r, i_b in qd.ndrange(rigid_info.roots_link_idx.shape[0], dyn_state.links.pos.shape[1]):
        ...
        for i_l_ in range(i_l_end - i_l_root):
            ...
            if dyn_info.links.root_idx[I_l] == i_l_root:
                func_forward_kinematics_link_bw(i_l, i_b, dyn_state, dyn_info, rigid_info, rigid_config)
...
def func_cd_contraction_bw(
    ...
    dofs [dof_lo, dof_hi): accumulates cd_*_bw[i_slot].grad into cdof_{ang,vel}.grad and vel.grad."""
    g_cd_vel = dyn_state.links.cd_vel_bw.grad[i_l, i_slot, i_b]
    g_cd_ang = dyn_state.links.cd_ang_bw.grad[i_l, i_slot, i_b]
    for i_d in range(dof_lo, dof_hi):
        ...
        for k in qd.static(range(3)):
            dyn_state.dofs.cdof_vel.grad[i_d, i_b][k] = (
                dyn_state.dofs.cdof_vel.grad[i_d, i_b][k] + g_cd_vel[k] * dof_vel
            )
            dyn_state.dofs.cdof_ang.grad[i_d, i_b][k] = (
                dyn_state.dofs.cdof_ang.grad[i_d, i_b][k] + g_cd_ang[k] * dof_vel
            )
        dot_vel = cdof_vel[0] * g_cd_vel[0] + cdof_vel[1] * g_cd_vel[1] + cdof_vel[2] * g_cd_vel[2]
        dot_ang = cdof_ang[0] * g_cd_ang[0] + cdof_ang[1] * g_cd_ang[1] + cdof_ang[2] * g_cd_ang[2]
...
def func_forward_velocity_link_bw(
    ...
    """Reverse the forward velocity of link i_l of env i_b.

    Its velocity grads flow back to its dof velocities, its motion subspace and its parent's velocity grads (see
    ...
    for i_j_rev in range(n_joints):
        ...
        # --- Reverse the joint-space contraction into cd_*_bw[next].
        # Forward (FREE contracts only its angular dofs here, the linear ones go through slot curr):
        #   _vel = cdof_vel[d] * vel[d], atomic_add(cd_vel_bw[next], _vel)
        #   _ang = cdof_ang[d] * vel[d], atomic_add(cd_ang_bw[next], _ang)
        dof_lo = dof_start + 3 if joint_type == gs.JOINT_TYPE.FREE else dof_start
        func_cd_contraction_bw(i_b, i_l, i_slot_next, dof_lo, dof_end, dyn_state)
        ...
        for k in qd.static(range(3)):
            ...
            dyn_state.links.cd_ang_bw.grad[i_l, i_slot, i_b][k] = (
                dyn_state.links.cd_ang_bw.grad[i_l, i_slot, i_b][k] + g_cd_ang_next[k]
            )
        # consume next
        for k in qd.static(range(3)):
            dyn_state.links.cd_vel_bw.grad[i_l, i_slot_next, i_b][k] = 0.0
            dyn_state.links.cd_ang_bw.grad[i_l, i_slot_next, i_b][k] = 0.0

        # --- Reverse motion_cross_motion:
        # Forward: (cdofd_ang[i_d], cdofd_vel[i_d]) =
        #     motion_cross_motion(cd_ang_bw[curr], cd_vel_bw[curr], cdof_ang[i_d], cdof_vel[i_d])
        # over the same dof range as the contraction above (FREE applies it to its angular dofs only).
        s_ang_primal = dyn_state.links.cd_ang_bw[i_l, i_slot, i_b]
        s_vel_primal = dyn_state.links.cd_vel_bw[i_l, i_slot, i_b]
        for i_d in range(dof_lo, dof_end):
            ...
            for k in qd.static(range(3)):
                dyn_state.links.cd_ang_bw.grad[i_l, i_slot, i_b][k] = (
                    dyn_state.links.cd_ang_bw.grad[i_l, i_slot, i_b][k] + g_cd_ang[k]
                )
                dyn_state.links.cd_vel_bw.grad[i_l, i_slot, i_b][k] = (
                    dyn_state.links.cd_vel_bw.grad[i_l, i_slot, i_b][k] + g_cd_vel[k]
                )
                dyn_state.dofs.cdof_ang.grad[i_d, i_b][k] = dyn_state.dofs.cdof_ang.grad[i_d, i_b][k] + g_cdof_ang[k]
                dyn_state.dofs.cdof_vel.grad[i_d, i_b][k] = dyn_state.dofs.cdof_vel.grad[i_d, i_b][k] + g_cdof_vel[k]
            # consume cdofd_*.grad[i_d]
            for k in qd.static(range(3)):
                dyn_state.dofs.cdofd_ang.grad[i_d, i_b][k] = 0.0
                dyn_state.dofs.cdofd_vel.grad[i_d, i_b][k] = 0.0
        if joint_type == gs.JOINT_TYPE.FREE:
            # Linear dofs: the forward writes constant-zero cdofd, so the reverse is a no-op, and the grads are consumed
            # so they do not leak into the next substep
        ...
        # --- Reverse the FREE linear contraction into cd_*_bw[curr].
        ...
        if joint_type == gs.JOINT_TYPE.FREE:
            func_cd_contraction_bw(i_b, i_l, i_slot, dof_start, dof_start + 3, dyn_state)

    # --- Step 1 (initial cvel setup) reverse:
    # Forward: cd_*_bw[i_l, 0, i_b] = parent.cd_*[i_parent, i_b] (if i_parent != -1) else 0
    # Reverse: parent.cd_*.grad[i_parent] += cd_*_bw[i_l, 0].grad, then slot 0 is consumed
...
def kernel_manual_forward_velocity_bw(
    ...
    """
    # Leaf to root, one thread per root (see kernel_manual_forward_kinematics_bw)
    qd.loop_config(
    ...
    for i_r, i_b in qd.ndrange(rigid_info.roots_link_idx.shape[0], dyn_state.links.pos.shape[1]):
        ...
        for i_l_ in range(i_l_end - i_l_root):
            ...
            I_l = [i_l, i_b] if qd.static(rigid_config.batch_links_info) else i_l
            if dyn_info.links.root_idx[I_l] == i_l_root:
                func_forward_velocity_link_bw(i_l, i_b, dyn_state, dyn_info, rigid_info, rigid_config)
